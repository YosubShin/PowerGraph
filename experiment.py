...
for run in range(1):  # Do experiment X many times
    for num_iteration in num_iterations:
        ...
        lines = lines.split('\n')
        vid_str = ' '.join(map(lambda l: '--source=' + l.split('\t')[0], filter(lambda x: len(x) > 0, lines)))
        for algorithm in algorithms:
            ...
            for partitioning_strategy in partitioning_strategies:
                ...
                #            stderr=STDOUT) 
                check_call(['mpiexec', '-n', str(num_hosts), '-hostfile', hostfile_path, algorithm_path, '--graph=%s' % graph_path, '--format=snap', vid_str, '--graph_opts=ingress=%s' % partitioning_strategy],
                           stderr=STDOUT) 
