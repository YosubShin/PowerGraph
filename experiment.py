...
for run in range(1):  # Do experiment X many times
    for num_iteration in num_iterations:
    	lines = subprocess.check_output(['shuf', '-n', num_iteration, '%s/lab_data_lj' % home_path])
	    lines = lines.split('\n')
	    vid_str = ' '.join(map(lambda l: l.split('\t')[0]))
        for algorithm in algorithms:
            ...
            for partitioning_strategy in partitioning_strategies:
                ...
                #            stderr=STDOUT) 
				check_call(['mpiexec', '-n', str(num_hosts), '-hostfile', hostfile_path, algorithm_path, '--graph=%s' % graph_path, '--format=snap', '--source=%s' % vid_str, '--graph_opts=ingress=%s' % partitioning_strategy],
                           stderr=STDOUT) 
