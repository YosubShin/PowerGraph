    ...
    public:
     ...
     procid_t edge_to_proc_greedy_and_topology (const vertex_id_type source,
         ...
         for (size_t i = 0; i < numprocs; ++i) {
	       ...
	       + (2.0 * shortest_dist - (src_dist + dst_dist)) / (2.0 * epsilon + shortest_dist) // minimize src_dist + dst_dist
                     + (shortest_dist - std::abs(src_dist - dst_dist)) / (2.0 * epsilon + shortest_dist); // minimize src_dist and dst_dist difference
         }
         maxscore = *std::max_element(proc_score.begin(), proc_score.end());
