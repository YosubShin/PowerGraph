         ...
         for (size_t i = 0; i < dc.numprocs(); ++i) {
             ...
             for (size_t j = 0; j < dc.numprocs(); ++j) {
                 ...
                 for (size_t k = 0; k < dc.numprocs(); ++k) {
                     ...
                     const size_t dst_can_dist = coords_pair_dist(dst_coord, candidate_coord);
                     coords2score[key] = ((2.0 * src_dst_dist - (src_can_dist + dst_can_dist)) / (epsilon + src_dst_dist)) * dc.topology_weight();
